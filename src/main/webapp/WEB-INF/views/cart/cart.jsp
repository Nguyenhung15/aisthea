        ...
        <c:choose>
            <c:when test="${empty sessionScope.cart or sessionScope.cart.items.isEmpty()}">
                <div class="glass-card rounded-2xl p-20 text-center max-w-2xl mx-auto mt-10">
                    <span class="material-symbols-outlined text-6xl text-slate-300 mb-6">shopping_bag</span>
                    <h2 class="font-serif text-3xl font-semibold text-slate-800 mb-4">Your cart is empty</h2>
                    <p class="text-slate-500 mb-10">Discover our latest collection and find something special.</p>
            ...
            <c:otherwise>
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                    <div class="lg:col-span-8 space-y-6">
                        <h1 class="font-serif text-4xl font-semibold text-slate-800 mb-8">Shopping Bag</h1>
                            ...
                            <div class="divide-y divide-sky-100">
                                <c:forEach var="item" items="${sessionScope.cart.items}">
                                    <div class="p-8 flex flex-col md:flex-row gap-8 items-center">
                                        <div class="w-32 h-40 flex-shrink-0 rounded-xl bg-slate-100 overflow-hidden shadow-sm">
                                            <img class="w-full h-full object-cover" src="${item.productImageUrl}" alt="${item.productName}" />
                                        </div>

                                        <div class="flex-grow space-y-2 text-center md:text-left">
                                            <h3 class="text-lg font-medium text-slate-900">
                                                <a href="${pageContext.request.contextPath}/product?action=view&id=${item.productId}" class="hover:text-accent-blue transition-colors">
                                                    ${item.productName}
                                                </a>
                                            </h3>
                                            <p class="text-xs text-slate-500 italic uppercase tracking-wider">
                                                Color: ${item.color} <span class="mx-2">|</span> Size: ${item.size}
                                            </p>
                                            <div class="pt-4">
                                        ...
                                        <div class="flex flex-col items-center md:items-end gap-4 min-w-[150px]">
                                            <form action="${pageContext.request.contextPath}/cart" method="POST">
                                                ...
                                                <div class="flex items-center border border-slate-200 rounded-lg bg-white/50 p-1">
                                                    <button type="button" onclick="decreaseQty(this)" class="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-accent-blue transition-colors">
                                                    ...
                                                    <button type="button" onclick="increaseQty(this)" class="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-accent-blue transition-colors">
                        ...
                        <a href="${pageContext.request.contextPath}/" class="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-accent-blue transition-all">
                    ...
                    <div class="lg:col-span-4">
                        <div class="sticky top-32 glass-card rounded-xl p-8 border border-slate-200 shadow-sm">
                            <h2 class="font-serif mb-8 text-slate-800 tracking-wide font-semibold text-2xl">Summary</h2>
                            ...
                            <div class="space-y-4 mb-8">
                                ...
                                <div class="pt-6 border-t border-sky-100 flex justify-between items-baseline">
                                    <span class="text-xl font-bold text-slate-900">Total</span>
                                    <div class="text-right">
                                        <p class="text-2xl font-bold text-accent-blue">
                                            <fmt:formatNumber value="${sessionScope.cart.totalPrice}" type="currency" currencyCode="VND" maxFractionDigits="0" />
                                        </p>
                                        <p class="text-[9px] text-slate-400 uppercase tracking-widest mt-1">VAT Included</p>
                                    </div>
                                </div>
                            </div>

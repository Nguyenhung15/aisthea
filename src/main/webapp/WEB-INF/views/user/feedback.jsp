...
<head>
  ...
  <style>
    .glass-island { background: rgba(255,255,255,0.65); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.8); box-shadow: 0 20px 40px -10px rgba(0,86,179,0.05); }
    .glass-input { background: rgba(255,255,255,0.4); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.6); transition: all 0.3s ease; }
    .glass-input:focus { background: rgba(255,255,255,0.8); border-color: #0056b3; box-shadow: 0 0 0 4px rgba(0,86,179,0.1); outline: none; }
    .marble-texture { background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 400 400' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)' opacity='0.05'/%3E%3C/svg%3E"); opacity:0.4; pointer-events:none; }
    ::-webkit-scrollbar { width: 8px; }
    ::-webkit-scrollbar-track { background: transparent; }
    ::-webkit-scrollbar-thumb { background: rgba(0,86,179,0.2); border-radius: 4px; }
    .product-review-section { transition: all 0.3s ease; }
    .star-span { transition: all 0.2s cubic-bezier(0.4,0,0.2,1); cursor: pointer; user-select: none; }
...
<body class="bg-ethereal-sky font-body min-h-screen text-slate-800 relative selection:bg-primary/20 selection:text-primary">
  <div class="fixed inset-0 z-0">
    ...
    <div class="absolute top-0 right-0 w-[500px] h-[500px] bg-blue-200/20 rounded-full blur-3xl translate-x-1/2 -translate-y-1/2"></div>
    <div class="absolute bottom-0 left-0 w-[600px] h-[600px] bg-sky-100/30 rounded-full blur-3xl -translate-x-1/4 translate-y-1/4"></div>
  </div>
          ...
          <div class="relative z-10 max-w-3xl mx-auto">
            ...
            <header class="mb-10 border-b border-slate-200/60 pb-6">
              <div class="flex items-center gap-2 mb-3">
                ...
                <span class="text-[10px] font-bold text-accent-gold uppercase tracking-widest">Đơn hàng #${orderId}</span>
              </div>
              <h1 class="font-display font-bold text-3xl text-slate-900 mb-2 tracking-tight">Đánh giá sản phẩm</h1>
              <p class="text-slate-500 font-light text-sm tracking-wide">
                <c:choose>
                  <c:when test="${fn:length(orderItems) > 1}">Điền đánh giá cho ${fn:length(orderItems)} sản phẩm bên dưới rồi nhấn Gửi một lần.</c:when>
                  <c:otherwise>Chia sẻ trải nghiệm của bạn để giúp cộng đồng AISTHÉA lựa chọn tốt hơn.</c:otherwise>
                </c:choose>
              </p>
            </header>
            ...
            <form action="${pageContext.request.contextPath}/feedback" method="POST"
                  id="feedbackForm" enctype="multipart/form-data" class="space-y-10" novalidate>
              ...
              <c:forEach var="item" items="${orderItems}" varStatus="st">
                ...
                <div class="product-review-section fade-in rounded-2xl border border-slate-100 bg-white/50 shadow-sm overflow-hidden"
                     style="animation-delay: ${st.index * 0.08}s">

                  <%-- ── Product header ── --%>
                  <div class="flex items-center gap-4 p-5 border-b border-slate-100 bg-gradient-to-r from-slate-50/80 to-white/40">
                    <div class="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 shadow-sm border border-white/80">
                      <img src="${item.imageUrl}" alt="${item.productName}"
                           class="w-full h-full object-cover"
                           onerror="this.src='https://placehold.co/56x56/f0f2f5/9ca3af?text=N/A'">
                    </div>
                    <div class="flex-1 min-w-0">
                      <h3 class="font-display font-bold text-[15px] text-slate-900 truncate">${item.productName}</h3>
                      <div class="flex items-center gap-2 mt-1">
                        <span class="inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full
                  ...
                  <c:if test="${not empty existFb}">
                    ...
                    <c:if test="${not empty existFb.imageUrl}">
                      <input type="hidden" name="existingImageUrl_${pid}" id="existingImageUrl_${pid}" value="${existFb.imageUrl}" />
                    </c:if>
                  </c:if>
                  ...
                  <div class="p-6 space-y-6">
                      ...
                      <div class="flex items-center justify-center gap-2" id="starContainer_${pid}">
                        <c:forEach begin="1" end="5" var="i">
                          <span class="star-span material-symbols-outlined text-4xl"
                                ...
                                onmouseout="unhoverStar('${pid}')"
                                onclick="clickStar(this)">star</span>
                        </c:forEach>
                        <span class="ml-3 text-sm font-medium text-slate-400" id="ratingLabel_${pid}">Chọn số sao</span>
                      </div>
                    </div>
                    ...
                    <div class="space-y-2">
                      <div class="flex justify-between items-baseline">
                        <label class="text-xs font-bold text-slate-500 uppercase tracking-wider">Nhận xét chi tiết</label>
                    ...
                    <div class="space-y-2">
                      <div class="flex justify-between items-baseline">
                        ...
                        <span class="text-[10px] text-slate-400 italic">Tối đa 5 MB — không bắt buộc</span>
                      </div>
                      ...
                      <div id="dropZone_${pid}"
                        ...
                        <div id="dropDefault_${pid}"
                          ...
                          <span class="material-symbols-outlined text-primary text-[24px]" style="font-variation-settings:'FILL' 0;">add_a_photo</span>
                        ...
                        <div id="dropPreview_${pid}"
                          ...
                          <div class="relative inline-block">
                            <img id="previewImg_${pid}"
                                 ...
                                 alt="Preview" class="max-h-36 max-w-full rounded-xl shadow-md object-contain border border-white">
                            <button type="button" onclick="event.stopPropagation(); clearImg('${pid}')"
                          ...
                          <p id="previewName_${pid}" class="text-[11px] text-slate-500 font-medium">
              ...
              <%-- ── Submit bar ── --%>
              <div class="sticky bottom-6 z-20 pt-4">
                <div class="glass-island rounded-2xl px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-lg">
                  <p class="text-[11px] text-slate-400 text-center sm:text-left">
                    Đánh giá sẽ hiển thị sau khi được AISTHÉA xét duyệt.
                  </p>
